...
// setcookie("uniqueFarmers", serialize($uniqueFarmers), time() + (86400), "/", "localhost"); 
// setcookie("uniqueFarmersSubTotal", serialize($uniqueFarmersSubTotal), time() + (86400), "/", "localhost"); 
// setcookie("products", serialize($products), time() + (86400), "/", "localhost"); 



print_r($uniqueFarmersSubTotal);
...
foreach($uniqueFarmers as $key => $value) {
    $fid = $value;
    $sql = "INSERT INTO orders (cid, fid, txnid, amount, status) VALUES ('{$cid}', '{$fid}', '{$txnid}', '{$uniqueFarmersSubTotal[$key]}', 'Payment Pending')";
    $result = mysqli_query($con, $sql);
}
